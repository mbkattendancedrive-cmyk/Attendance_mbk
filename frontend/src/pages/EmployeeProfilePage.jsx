    ...
    <div className="space-y-6 max-w-4xl mx-auto animate-fade-in pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200/80">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Complete corporate record, personal credentials, and official identity badge.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-5 pb-6 border-b border-slate-100">
          {user.profilePhoto ? (
            <img
              ...
              alt={user.name}
              className="w-24 h-24 rounded-full object-cover border-2 border-slate-200 shadow-md shrink-0"
            />
          ) : (
            <div className="w-24 h-24 rounded-full bg-slate-900 text-white font-bold flex items-center justify-center text-3xl shadow-md shrink-0">
          ...
          <div className="text-center sm:text-left space-y-1.5 flex-1">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
              ...
              <span className={user.status === 'Active' ? 'badge-success text-xs' : 'badge-danger text-xs'}>
                {user.status || 'Active Employee'}
              </span>
            </div>
            <p className="text-sm font-bold text-indigo-600">{user.designation} • <span className="text-slate-700 font-semibold">{user.department} Department</span></p>
            
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 pt-1">
              <button
                onClick={() => copyToClipboard(user.employeeId, 'Employee ID')}
                className="font-mono text-xs text-slate-700 bg-slate-100 hover:bg-slate-200 px-2.5 py-1 rounded-lg border border-slate-200 font-bold inline-flex items-center gap-1.5 transition-colors"
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/70 space-y-1">
                ...
                <span className="font-bold text-slate-900 text-sm block truncate">{user.department}</span>
              </div>

              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/70 space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Designation</span>
                <span className="font-bold text-slate-900 text-sm block truncate">{user.designation}</span>
              </div>

              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/70 space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Date of Joining</span>
                <span className="font-bold text-slate-900 text-sm block truncate">{formatDate(user.joiningDate)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 pt-4 border-t border-slate-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/70 space-y-1">
                ...
                <span className="font-bold text-slate-900 text-sm block truncate">{user.name}</span>
              </div>

              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/70 space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Date of Birth</span>
                <span className="font-bold text-slate-900 text-sm block truncate">{formatDate(user.dateOfBirth)}</span>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-rose-50/50 border border-rose-200/60 space-y-1">
                ...
                <span className="font-bold text-rose-800 text-sm block truncate">{user.bloodGroup || 'O+'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 pt-4 border-t border-slate-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/70 space-y-1">
                ...
                <span className="font-bold text-slate-900 text-sm block truncate">{user.phone || 'N/A'}</span>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/70 space-y-1">
                ...
                <span className="font-bold text-slate-900 text-sm block truncate">{user.emergencyContact || 'N/A'}</span>
              </div>
            </div>
          ...
          <div className="space-y-3 pt-4 border-t border-slate-100">
            ...
            <div className="p-4 rounded-xl bg-amber-50/70 border border-amber-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-amber-900 block">Current Dashboard Access Password</span>
                <p className="text-[11px] text-amber-700 font-medium">Use your Employee ID or Email and this password to sign in to your workspace.</p>
              </div>
